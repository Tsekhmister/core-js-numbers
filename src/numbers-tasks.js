...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const distanceBetweenX = x2 - x1;
  const distanceBetweenY = y2 - y1;
  return Math.sqrt(
    distanceBetweenX * distanceBetweenX + distanceBetweenY * distanceBetweenY
  );
}
...
function getFibonacciNumber(index) {
  ...
  for (let i = 2; i <= index; i += 1) {
    const next = prevNumber + currentNumber;
...
function getSumOfNumbers(x1, x2, x3) {
  const sum = [x1, x2, x3].reduce((acc, el) => acc + el, 0);
...
function getMaxNumber(firstNumber, secondNumber) {
  return Math.max.call(null, firstNumber, secondNumber);
}
